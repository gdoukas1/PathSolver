...
class SequentialSearch(SearchBaseClass, ABC):
    ...
    def successor_to_node(self, successor, node_current, cost=True):
    ...
    def heuristic_function(self, node_current, heuristic_option):
        """
        ...
        node_center = self.get_node_information(node_current)
        goal_center = self.get_goal_information()
        distance_x = abs(goal_center[0] - node_center[0])
        distance_y = abs(goal_center[1] - node_center[1])

        # Euclidean Heuristic
        if heuristic_option == 1:
            distance = math.sqrt(distance_x ** 2 + distance_y ** 2)
        # Manhattan Distance variation Heuristic
        elif heuristic_option == 2:
            distance = distance_x + distance_y + math.sqrt(2) * min(distance_x, distance_y)
        else:
            distance = 0
        return distance

    def evaluation_function(self, node_current, heuristic_option, heuristic_weight):
        """
        f(x) = g(x) + w * h(x)
        """
        g = self.cost_function(node_current)
        h = self.heuristic_function(node_current, heuristic_option)
        f = g + heuristic_weight * h
        return f, h

    def Iterative_Deepening_Astar(self, node_current, heuristic_option, heuristic_weight, count, fmax):
        queue = []
        tie_breaker = 0
        node_initial = node_current
        while True:
            ...
            for primitive_successor in node_current.get_successors():
                ...
                collision_flag, child = self.successor_to_node(successor=primitive_successor, node_current=node_current)
                if not collision_flag:
                    node_f, node_h = self.evaluation_function(child, heuristic_option, heuristic_weight)
                    queue.append((node_f, node_h, tie_breaker, primitive_successor, node_current))
                    tie_breaker += 1
            queue.sort(reverse=True)
            temp_node = queue.pop()
            if temp_node[0] <= fmax:
                node_current = temp_node[4]
                collision_flag, child = self.take_step(successor=temp_node[3], node_current=node_current)

                # check whether goal is reached
                goal_flag = self.goal_reached(successor=temp_node[3], node_current=node_current)
                # if goal is reached, return back with the solution path
                if goal_flag:
                    break
                if len(queue) == 0:
                    count = len(self.list_status_nodes) - 1
                    node_f, node_h = self.evaluation_function(node_initial, heuristic_option, heuristic_weight)
                    return None, count, node_h, 0
                # go to next node
                node_current = child
            else:
                new_fmax = temp_node[0]
                new_count = count + len(self.list_status_nodes) - 1
                path_solution, new_count, node_h, g_cost = self.Iterative_Deepening_Astar(node_current=node_initial,
                                                                                          heuristic_option=heuristic_option,
                                                                                          heuristic_weight=heuristic_weight,
                                                                                          count=new_count,
                                                                                          fmax=new_fmax)
                return path_solution, new_count, node_h, g_cost
        # solution
        new_count = count + len(self.list_status_nodes) - 1
        node_f, node_h = self.evaluation_function(node_initial, heuristic_option, heuristic_weight)
        g_cost = child.cost
        path_solution = self.get_node_path(child)
        return path_solution, new_count, node_h, g_cost

    def execute_search(self, time_pause, heuristic_option, heuristic_weight):
        ...
        """
        fmax = self.evaluation_function(node_current=node_initial, heuristic_option=heuristic_option,
                                        heuristic_weight=heuristic_weight)[0]
        count = 0
        path_solution, count, node_h, g_cost = self.Iterative_Deepening_Astar(node_current=node_initial,
                                                                              heuristic_option=heuristic_option,
                                                                              heuristic_weight=heuristic_weight,
                                                                              count=count,
                                                                              fmax=fmax)
        return path_solution, count, node_h, g_cost
